...
export default function RoomProvider({ children, roomId }: LayoutProps) {
    ...
    useEffect(() => {
        ...
        const subscribeToRoom = async () => {
            try {
                ...
                // First, check if room exists immediately
                ...
                unsubscribe = onSnapshot(roomRef,
                    (snapshot) => {
                        if (snapshot.exists()) {
                            ...
                            if (roomData.status === 'closed' || roomData.status === 'ended') {
    ...
    return (
        <RoomContext.Provider value={{ room, isHost }}>
            <RoomManager>
                {children}
            </RoomManager>
